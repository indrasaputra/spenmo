...
type UserQuery struct {
	...
	// eager-loading edges.
	withUserWallets *UserWalletQuery
	withUserCards   *UserCardQuery
...
// QueryUserWallets chains the current query on the "user_wallets" edge.
func (uq *UserQuery) QueryUserWallets() *UserWalletQuery {
	query := &UserWalletQuery{config: uq.config}
...
// QueryUserCards chains the current query on the "user_cards" edge.
func (uq *UserQuery) QueryUserCards() *UserCardQuery {
	query := &UserCardQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withUserWallets: uq.withUserWallets.Clone(),
		withUserCards:   uq.withUserCards.Clone(),
...
// WithUserWallets tells the query-builder to eager-load the nodes that are connected to
// the "user_wallets" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserWallets(opts ...func(*UserWalletQuery)) *UserQuery {
	query := &UserWalletQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserWallets = query
	return uq
}

// WithUserCards tells the query-builder to eager-load the nodes that are connected to
// the "user_cards" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithUserCards(opts ...func(*UserCardQuery)) *UserQuery {
	query := &UserCardQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserCards = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withUserWallets != nil,
			uq.withUserCards != nil,
		}
	)
	...
	if query := uq.withUserWallets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int64]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserWallets = []*UserWallet{}
		}
		query.Where(predicate.UserWallet(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserWalletsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.UserWallets = append(node.Edges.UserWallets, n)
		}
	}

	if query := uq.withUserCards; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int64]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserCards = []*UserCard{}
		}
		query.Where(predicate.UserCard(func(s *sql.Selector) {
			s.Where(sql.InValues(user.UserCardsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.UserCards = append(node.Edges.UserCards, n)
		}
	}
