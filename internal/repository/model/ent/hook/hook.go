...
// The UserCardFunc type is an adapter to allow the use of ordinary
// function as UserCard mutator.
type UserCardFunc func(context.Context, *ent.UserCardMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserCardFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.UserCardMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserCardMutation", m)
	}
	return f(ctx, mv)
}
