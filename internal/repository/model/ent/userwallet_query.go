...
type UserWalletQuery struct {
	...
	// eager-loading edges.
	withUser            *UserQuery
	withUserWalletCards *UserCardQuery
...
func (uwq *UserWalletQuery) QueryUser() *UserQuery {
	query := &UserQuery{config: uwq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uwq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := uwq.sqlQuery(ctx)
		...
		fromU = sqlgraph.SetNeighbors(uwq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryUserWalletCards chains the current query on the "user_wallet_cards" edge.
func (uwq *UserWalletQuery) QueryUserWalletCards() *UserCardQuery {
	query := &UserCardQuery{config: uwq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uwq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uwq *UserWalletQuery) Clone() *UserWalletQuery {
	...
	return &UserWalletQuery{
		...
		withUser:            uwq.withUser.Clone(),
		withUserWalletCards: uwq.withUserWalletCards.Clone(),
...
func (uwq *UserWalletQuery) WithUser(opts ...func(*UserQuery)) *UserWalletQuery {
	query := &UserQuery{config: uwq.config}
...
// WithUserWalletCards tells the query-builder to eager-load the nodes that are connected to
// the "user_wallet_cards" edge. The optional arguments are used to configure the query builder of the edge.
func (uwq *UserWalletQuery) WithUserWalletCards(opts ...func(*UserCardQuery)) *UserWalletQuery {
	query := &UserCardQuery{config: uwq.config}
	for _, opt := range opts {
		opt(query)
	}
	uwq.withUserWalletCards = query
	return uwq
}
...
func (uwq *UserWalletQuery) sqlAll(ctx context.Context) ([]*UserWallet, error) {
	...
	if query := uwq.withUser; query != nil {
	...
	if query := uwq.withUserWalletCards; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int64]*UserWallet)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.UserWalletCards = []*UserCard{}
		}
		query.Where(predicate.UserCard(func(s *sql.Selector) {
			s.Where(sql.InValues(userwallet.UserWalletCardsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "wallet_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.UserWalletCards = append(node.Edges.UserWalletCards, n)
		}
	}
